...
## no_tt = TRUE --------------------------------------
r.zc_t0 <- function(data_in, varnames, folds, learners, bounded = TRUE) {
  tr_vals <- expand.grid(tt=c(0), r=c(0,1))
  r_ztc <- matrix(nrow = nrow(data_in), ncol = nrow(tr_vals))
  colnames(r_ztc) <- (glue("r({tr_vals$r}|z,t{tr_vals$tt},c)"))
  v <- 1
...
r.mc_t0 <- function(data_in, varnames, folds, learners, bounded = TRUE) {
  tr_vals <- expand.grid(tt=c(0), r=c(0,1))
  r_mtc <- matrix(nrow = nrow(data_in), ncol = nrow(tr_vals))
  colnames(r_mtc) <- (glue("r({tr_vals$r}|m,t{tr_vals$tt},c)"))
  v <- 1
...
r.zmc_t0 <- function(data_in, varnames, interact, folds, learners, bounded = TRUE) {
  tr_vals <- expand.grid(tt=c(0), r=c(0,1))
  r_zmtc <- matrix(nrow = nrow(data_in), ncol = nrow(tr_vals))
  colnames(r_zmtc) <- (glue("r({tr_vals$r}|z,m,t{tr_vals$tt},c)"))
  v <- 1
  for (v in seq_along(folds)) {
    ...
    if (interact == TRUE) {
      alist <- crossfit(train, valid_list,
                        varnames$R,
                        c(varnames$Z, varnames$M, varnames$MZ,
                          varnames$C), varnames,
                        type = c("binomial"), learners, "ID", bounded)
    }
    if (interact == FALSE) {
...
# # alternatively
# rt.c <- function(t_c, r_tc) {
#   tr_vals <- expand.grid(tt=c(0,1), r=c(0,1))
#   rt_c <- matrix(nrow = nrow(r_c), ncol = nrow(tr_vals))
#   colnames(rt_c) <- glue("p(t{tr_vals$tt},r{tr_vals$r}|c)")
#   for (jj in 1:nrow(tr_vals)) {
#     tt <- tr_vals$tt[jj]
#     r <- tr_vals$r[jj]
#     rt_c[, glue("p(t{tt},r{r}|c)")] <-  t_c[, glue("t({tt}|c)")] * r_tc[, glue("r({r}|t{tt},c)")]
#   }
#   rt_c
# }
