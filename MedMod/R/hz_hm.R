...
h.zm <- function(data_in, varnames, Mfamily, interact, folds, learners, bounded = FALSE, full.sample) {
  ...
  for (v in seq_along(folds)) {
    ...
    train <- stack_data(origami::training(data_in, folds[[v]]), varnames$M)
    valid <- origami::validation(data_in, folds[[v]])
    ...
    if (full.sample==FALSE) {
      for(jj in 1:nrow(tr_vals)) {
        ...
        p_zmc <- crossfit(train[predict_subset, ], valid_list[jj],
                          "Lambda",
                          c(varnames$M, varnames$Z, varnames$C), varnames,
                          type = "binomial", learners,
                          IDname = "tmp_id",
                          bounded = TRUE)
        p_mc <- crossfit(train[predict_subset, ], valid_list[jj],
                         "Lambda",
                         c(varnames$M, varnames$C), varnames,
                         type = "binomial", learners,
                         IDname = "tmp_id",
                         bounded = TRUE)
        h_zm[folds[[v]]$validation_set, glue("h_zm(t{tr_vals$tt[jj]},r{tr_vals$r[jj]})")] <- p_zmc$preds[, 1] * (1 - p_mc$preds[, 1]) / ( (1 - p_zmc$preds[, 1])*p_mc$preds[, 1] )
      }
    }
...
h.zm.joint <- function(r, rstar, tt, tr_c, r_zmtc) {
  if (any(grepl("m", colnames(r_zmtc)))) {
    ...
    iorw_joint <- bound( tr_c[, glue("p(t{tt},r{r}|c)")]*r_zmtc[, glue("r({rstar}|z,t{tt},c)")] ) /
      bound(tr_c[, glue("p(t{tt},r{rstar}|c)")]*r_zmtc[, glue("r({r}|z,t{tt},c)")])
  }
...
stack_data <- function(data_in, Mediators) {
